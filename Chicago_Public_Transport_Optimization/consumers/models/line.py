...
class Line:
    ...
    def process_message(self, message):
        ...
        if message.topic == "^com.udacity.connect.stations.": # Set the conditional correctly to the stations Faust Table
            try:
                value = json.loads(message.value())
                self._handle_station(value)
            except Exception as e:
                logger.fatal("bad station? %s, %s", value, e)
        elif message == "^com.udacity.station.arrivals.":
            self._handle_arrival(message)
        elif message == "^com.udacity.station.turnstile":
